...
// router này dùng đê hứng response trả về từ callbackURL
router.get('/google/callback', (req, res, next) => {
  ...
  res.redirect(`http://127.0.0.1:5173/login-google/${(req?.user as any)?.id}`)
})
...
router.get('/facebook/callback', (req, res, next) => {
  ...
  , (req, res) => {
    res.redirect(`http://127.0.0.1:5173/login-facebook/${(req?.user as any)?.id}`)
  }
)

router.get('/github', passport.authenticate('github',{ scope: 'user:email', session: false, prompt: 'select_account' }));

router.get('/github/callback', (req, res, next) => {
  passport.authenticate('github', (error: any, profile: any) => {
    req.user = profile,
      next()
  })(req, res, next)
}, (req, res) => {

  res.redirect(`http://127.0.0.1:5173/login-github/${(req?.user as any)?.id}`)
}
)
